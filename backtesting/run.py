...
from backtesting.engine import run_backtest, BacktestResult, THRESHOLDS
...
def save_csv(results: list[BacktestResult], path: str):
    fieldnames = ["Token", "Strategy", "TF", "PF", "WR%", "Trades", "MaxDD%", "NetPft%", "Status"]
    ...
    for tf in timeframes:
        ...
        for strat_name in strategy_names:
            ...
            for token, df in ohlcv_map.items():
                try:
                    signals = strat_fn(df)
                    result = run_backtest(df, signals, token, strat_name, tf)
