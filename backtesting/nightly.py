...
def write_sizing_overrides(passing: list, htf: bool = False) -> None:
    ...
    tier_sizes = {tier: size_pct for _pf, tier, size_pct in SIZING_TIERS}
    tier_summary = ", ".join(
        f"{tier} PF>={pf} ({size}%)" for pf, tier, size in SIZING_TIERS
    )
    header = (
        ...
        "# Slots below the minimum PF tier are omitted (signal's suggested size applies).\n"
